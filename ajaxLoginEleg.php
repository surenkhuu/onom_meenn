...
if($finish=='')
{
	...
	else
	{
	    ...
	    mysql_select_db("onomcom_hepatitis");
	
		$rEleg = mysql_query("SELECT `rd` FROM `db_ubchtun` WHERE `rd`='".$rd."'");	
		
		$ifEleg=0;
		
		$ifEleg = mysql_num_rows($rEleg);
		
		if($ifEleg>0) {echo '[{"type":"eleg","msg":"['.$rd.'] Нууц үг авна уу."}]';}
		else
